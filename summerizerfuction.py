...
def summarization_sub(text:str)->str:
    model_file = r"\tinyllama_model\tinyllama-1.1b-1t-openorca.Q2_K.gguf"
    current_dir = os.path.dirname(__file__)
    model_path = current_dir + model_file

    config = ctransformers.AutoConfig.from_pretrained(model_path,local_files_only=True)
    ...
    llm = ctransformers.AutoModelForCausalLM.from_pretrained(model_path, model_type="llama",
            
            local_files_only=True,
            config = config
            )
    message = "please summarize paragraph into the main idea in 5 sentences:" + text
                
    txt_summarized = llm(prompt=message,max_new_tokens=200)
    return txt_summarized    
...
def summarization_main(chunks,num_of_sentences:int)->int:
    ...
    for chunk in chunks:
        if model_is_fed != True:
            ...
            print(chunk)
            text_input = chunk
            message = '''You are a summarizer. You write a summary of the input into a list of summarized sentences.
                Walk through the key points of this research paper step-by-step, explaining each point's significance. 
                Then summarize the entire document into {num_of_sentences} sentences for this chunk: {text_input}"
                '''

            model_is_fed = True # flag for the module to stop feeding the prompt, temporarily
            # prompt = llm.tokenize(message) # tokenize from ctransformer does not have apply chat template like usual, so chatml is out of question
            txt_summarized = llm(prompt=message,max_new_tokens=200)
...
# txt_summarized = summarization_main(chunks,2)
